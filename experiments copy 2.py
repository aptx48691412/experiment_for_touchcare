...
count=3
name_list=['okami','kodama','test','ueda','kanda','abe','akiyoshi','tainaka','asada','okami','tainaka']
...
def recording_movie(video,count):
    ...
    try:    # 撮影＝ループ中にフレームを1枚ずつ取得（qキーで撮影終了）
        ...
        #df_1119.iloc[1:,count]=np.array(center_new_list).T
...
if __name__ == '__main__':
    ...
    try:
        dssdds=pd.read_csv('csv/hand_{}_1117.csv'.format(name_list[0]))
    ...
    with open("csv/hand_{}_1117.csv".format(name_list[0]),"w",newline='') as f:
        writer=csv.writer(f)
        writer.writerow(['---'+str(i_fghj)+'---' for i_fghj in np.arange(1,13) ])
    
        
        #df_1119=pd.read_csv('./csv/hand_{}_1117.csv'.format(name_list[0]))
            #center_all_list=list()
            ...
            if key==27:
                ...
                for iuy in center_all_list:
                    if len(iuy) < np.max([len(ser) for ser in center_all_list]):
                        ...
                        #df_1119.iloc[1:,count]=np.array(iuy+list(np.zeros(np.max([len(tgb) for tgb in center_all_list])-len(iuy)))).T

                    else:
                        center_all_new_list.append(iuy)
                        #df_1119.iloc[1:,count]=np.array(iuy).T


                #print(center_all_list)
                writer.writerows(np.array(center_all_new_list).T)
                        
                #df_1119.iloc[1:,:]=np.array(center_all_new_list).T
                #df_1119.to_csv('./csv/hand_{}_1117.csv'.format(name_list[0]),index=False)
                ...
                p2=Process(target=speech2text_20211102_new.main, args=[all_start,video_list,count])
                ...
                try:
                    dfdf_cs=pd.read_csv('./csv/speech2txt_{}.csv'.format(video_list[count]),encoding='SHIFT-JIS',header=None)
                    idx_list=list()
                    idx_list_y=list()
